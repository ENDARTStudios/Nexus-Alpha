...
| — | **#058.11** Target-aware extraction gap analysis | 🟡 **fase 1 concluída (diagnóstico)** — `scripts/audit_extraction_gap.py` + `reports/extraction_gap_058_11.json`: 10 casos/54 sentenças-alvo, `converted_in_probe=0`; dominante `no_relation_pattern` (37/54 root não-verbal — definições nominais) + `predicate_unmapped` (11/54, verbos não-fato); **refutados**: `max_triples_truncation` (0/10), score (49/54 ≥0.6), anáfora/data/frase-longa; causa-raiz: `extract` exige `ROOT ∈ (VERB,AUX)` (`extractor.py:196`); fase 2: **F3 aplicada (#058.11.1)** — filtro determinístico `is_propositional_sentence` + `NOISE_SELECTORS` infobox/figure + `pre_filter_rejections` em `analyze_text` (414 testes); **rerun pós-F3 pendente** (gate: junk→<10 e `no_relation_pattern` ainda dominante → go F1; cai junto → parar e reavaliar #048.5) |
...
| #058.11 | Target-aware extraction gap analysis | v1.13.0 | 🟡 **fase 1 (diagnóstico) concluída** — classes: `no_relation_pattern` 7/10 casos (37/54 sentenças com root não-verbal) + `predicate_unmapped` 3/10; `converted=0`; truncamento/score/anáfora refutados; causa-raiz `ROOT ∈ (VERB,AUX)`; fase 2 = **F3 aplicada (#058.11.1)** (input limpo antes de F1), F1 copular gateado aguarda rerun pós-F3; F2 só acessório; não abrir #047.2/#045.3 amplos; não aumentar `max_triples` |
