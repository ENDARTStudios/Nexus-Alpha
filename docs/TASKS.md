...
| #058.11 | Target-aware extraction gap analysis | v1.13.0 | 🟡 **fase 1 (diagnóstico) concluída** — classes: `no_relation_pattern` 7/10 casos (37/54 sentenças com root não-verbal) + `predicate_unmapped` 3/10; `converted=0`; truncamento/score/anáfora refutados; causa-raiz `ROOT ∈ (VERB,AUX)`; fase 2 = decisão operador (F1 copular gateado, F3 input, F2 só acessório); não abrir #047.2/#045.3 amplos; não aumentar `max_triples` |
