...
| — | **#058.11** Target-aware extraction gap analysis | 📌 **aberta (próximo issue técnico)** — read-only/diagnóstico primeiro: por que sentença-alvo não vira tripla canônica (14 perguntas, classes de causa `max_triples_truncation`, `no_relation_pattern`, `anaphora_unresolved`, ...); evidência #048.5+#048.5a; não aumentar `max_triples` antes de medir; correção só após diagnóstico |
...
| #058.11 | Target-aware extraction gap analysis | v1.13.0 | 📌 **aberta** — read-only diagnóstico sentença→triplo (14 perguntas/classes de causa); evidência #048.5+#048.5a; não abrir #047.2/#045.3 sem a classe de causa; não aumentar `max_triples` sem medir truncamento |
